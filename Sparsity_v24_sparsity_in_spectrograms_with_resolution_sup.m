...
c7 = 1.3*c5;
c8 = -c1*c7^2*pi^2/8;
filter_phases_wihtout_chirp = {@(f) 1.6065*c1/2.*f.^2 ...
    @(f) c8*(log(abs(f-c7*pi/2))+log(abs(f+c7*pi/2)))...
    };
...
safe_range_of_freq = [-1 1].*1.71e-9/(1.6065*c1/(2*pi));
...
design_PD_BW = 8e9; % Photodetector bandwidth in Hz
design_ADC_BW = 8e9; % ADC Nyquist bandwidth in Hz
...
for filter_index = 1 : N_filters
    ...
    switch filter_index
        case 1
            PD_BW = 10.5e9; % Photodetector bandwidth in Hz
            ADC_BW = 10.5e9; % ADC Nyquist bandwidth in Hz
        case 2
            PD_BW = 8e9; % Photodetector bandwidth in Hz
            ADC_BW = 8e9; % ADC Nyquist bandwidth in Hz
    end
...
for row = 1 : N_filters
    ...
    switch row
        case 1
            arrow_tips_distance = 91; % in pixels 
        case 2
            arrow_tips_distance = 142; % in pixels
    end
